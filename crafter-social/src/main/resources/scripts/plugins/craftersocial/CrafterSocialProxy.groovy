...
logger.info("SOCIAL PROXY FILTER")
...
try {
    ...
    logger.info("SOCIAL PROXY: RewriteUrl")
    String proxyRequestUri = rewriteUrlFromRequest(request)
    proxyRequestUri = proxyRequestUri.replace("/social/","/")

    logger.info("SOCIAL PROXY: URI TO PROXY IS: "+proxyRequestUri)
    ...
    logger.info("SOCIAL PROXY: copy headers")

    copyRequestHeaders(request, proxyRequest)

    logger.info("SOCIAL PROXY: set forward")
    ...
    logger.info("SOCIAL PROXY: TICKET FOUND IS: "+ticket)
    if(ticket != null) {
        logger.info("SOCIAL PROXY: Adding cookie header for ticket to proxy request")
        proxyRequest.addHeader("Cookie", "ticket="+ticket+";")
    }


    logger.info("SOCIAL PROXY: execute :"+proxyRequest)

    if(request.getRequestURI().contains("/api/3/profile/avatar")) {
        logger.info("CACHEABLE REQUEST!!!!!!!!!!!!!!")
    }

    proxyResponse = proxyClient.execute(getTargetHost(request), proxyRequest);

    logger.info("SOCIAL PROXY: done execute")
...
private HttpHost getTargetHost(request) {
    // make this configurable
    return new HttpHost("console.craftersoftware.com", 443, "https")
}
